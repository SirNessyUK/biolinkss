...
class Router {
    ...
    public static function get_controller($controllerName, $path = '') {

        require_once APP_PATH . 'controllers/' . ($path != '' ? $path . '/' : null) . $controllerName . '.php';

        /* Create a new instance of the controller */
        $class = 'Altum\\Controllers\\' . $controllerName;
    ...
    public static function parse_method($controller) {
        ...
        if(isset(self::getParams()[0]) && method_exists($controller, self::getParams()[0])) {

            /* Make sure the method is not private */
            $reflection = new \ReflectionMethod($controller, self::getParams()[0]);
            if($reflection->isPublic()) {
                $method = self::getParams()[0];
