    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            if(empty($_SESSION['error'])) {
                ...
                if($this_account = Database::get(['user_id', 'email', 'name'], 'users', ['email' => $_POST['email']])) {
                    ...
                    $email_template = get_email_template(
                        ...
                        $this->settings->email_templates->lost_password_subject,
                        [
                            '{{LOST_PASSWORD_LINK}}' => url('reset-password/' . $_POST['email'] . '/' . $lost_password_code),
                            '{{NAME}}' => $this_account->name,
                            '{{WEBSITE_TITLE}}' => $this->settings->title
                        ],
                        $this->settings->email_templates->lost_password_body
                    );
