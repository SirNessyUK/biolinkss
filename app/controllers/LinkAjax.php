...
class LinkAjax extends Controller {
    ...
    private function create() {
        ...
        switch($_POST['type']) {
            ...
            case 'biolink':
                ...
                /* Check for subtype */
                if(isset($_POST['subtype']) && in_array($_POST['subtype'], ['link', 'soundcloud', 'youtube', 'twitch', 'vimeo'])) {
    ...
    private function create_link() {
        ...
        $_POST['url'] = isset($_POST['url']) ? get_slug(Database::clean_string($_POST['url'])) : false;
        ...
        if(!filter_var($_POST['location_url'], FILTER_VALIDATE_URL)) {
        ...
        if(empty($errors)) {
            ...
            /* Insert to database */
            $stmt = Database::$database->prepare("INSERT INTO `links` (`project_id`, `user_id`, `type`, `subtype`, `url`, `location_url`, `settings`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param('ssssssss', $_POST['project_id'], $this->user->user_id, $type, $subtype, $url, $_POST['location_url'], $settings, \Altum\Date::$date);
            $stmt->execute();
    ...
    private function create_biolink() {
        ...
        $stmt->bind_param('sssssss', $_POST['project_id'], $this->user->user_id, $type, $subtype, $url,  $settings, \Altum\Date::$date);
        $stmt->execute();
        $link_id = $stmt->insert_id;
        $stmt->close();
    ...
    private function create_biolink_link() {
        ...
        Response::json('', 'success', ['url' => url('link/' . $_POST['link_id'])]);
    }
    ...
    private function update_biolink() {
        ...
        /* Update the record */
        $stmt = Database::$database->prepare("UPDATE `links` SET  `url` = ?, `settings` = ? WHERE `link_id` = ?");
        $stmt->bind_param('sss', $url, $settings, $link->link_id);
        $stmt->execute();
        $stmt->close();
    ...
    private function update_link() {
        ...
        $stmt = Database::$database->prepare("UPDATE `links` SET `url` = ?, `location_url` = ?, `start_date` = ?, `end_date` = ? WHERE `link_id` = ?");
        $stmt->bind_param('sssss', $_POST['url'], $_POST['location_url'], $_POST['start_date'], $_POST['end_date'], $_POST['link_id']);
        $stmt->execute();
        $stmt->close();
    ...
    private function delete() {
        ...
        if(empty($errors)) {
            ...
            $stmt = Database::$database->prepare("DELETE FROM `links` WHERE `link_id` = ? AND `user_id` = ?");
            $stmt->bind_param('ss', $_POST['link_id'], $this->user->user_id);
            $stmt->execute();
            $stmt->close();

            /* Determine where to redirect the user */
            if($link->type == 'biolink' && $link->subtype != 'base') {
                $redirect_url = url('link/' . $link->biolink_id);
            } else {
