...
class AdminUserUpdate extends Controller {
    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            switch($_POST['package_id']) {
                case 'free':
                    ...
                    $package_settings = json_encode($this->settings->package_free);

                    break;

                case 'trial':

                    $package_settings = json_encode($this->settings->package_trial);
        ...
        $user->package = (new Package(['settings' => $this->settings]))->getPackageById($user->package_id);
