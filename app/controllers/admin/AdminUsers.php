...
class AdminUsers extends Controller {
    ...
    public function get() {
        ...
        while($row = $result->fetch_object()):
            ...
            /* Active Status badge */
            $row->active = $row->active ? '<span class="badge badge-pill badge-success">' . $this->language->global->active . '</span>' : '<span class="badge badge-pill badge-warning">' . $this->language->global->disabled . '</span>';

            /* Current Package */
            $package = (new Package(['settings' => $this->settings]))->getPackageById($row->package_id);

            $row->active .=  $package ? '&nbsp;' . '<span class="badge badge-pill badge-light" data-toggle="tooltip" title="' . $this->language->admin_users->tooltip->package . '">' . $package->name . '</span>' : null;

            $row->date = '<span data-toggle="tooltip" title="' . \Altum\Date::get($row->date, true) . '">' . \Altum\Date::get($row->date) . '</span>';
