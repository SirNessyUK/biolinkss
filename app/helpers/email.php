    ...
    if(!empty($settings->smtp->host)) {
        ...
        try {
            ...
            $replacers = [
                ...
                '{{HEADER}}'    => !empty($settings->logo) ? '<img src="' . url(UPLOADS_URL_PATH . 'logo/' . $settings->logo) . '" class="logo" />' : '<h2>' . $settings->title .  '</h2>'
            ];
        ...
        return send_server_mail($to, $settings->smtp->from, $title, $content);
    }
