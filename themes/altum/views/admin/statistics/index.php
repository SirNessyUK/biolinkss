...
<div class="d-flex flex-column flex-lg-row justify-content-between mb-5">
    <div>
        <h1><i class="fa fa-chart-bar fa-xs"></i> <?= sprintf($this->language->admin_statistics->header, '<span class="underline">' . $data->start_date . '</span>', '<span class="underline">' . $data->end_date . '</span>') ?></h1>
        ...
        <form class="form-inline" id="datepicker_form">
            ...
            <div class="input-group mr-sm-2">
                ...
                <input
                        ...
                        name="date_range"
                        value="<?= ($data->processed_date) ? $data->processed_date : '' ?>"
...
<div class="card border-0 mb-5">
    <div class="card-body">
        <h2><i class="fa fa-dollar-sign fa-xs text-muted"></i> <?= $this->language->admin_statistics->sales->header ?></h2>

        <?php $sales_data = $this->database->query("SELECT SUM(`amount`) AS `earnings`, `currency`, COUNT(`id`) AS `count` FROM `payments` WHERE `date` BETWEEN '{$data->start_date}' AND DATE_ADD('{$data->end_date}', INTERVAL 1 DAY) GROUP BY `currency` ") ?>
        ...
        <?php else: ?>
            ...
            <?php
            $logs_chart = [];
            $result = $this->database->query("SELECT COUNT(*) AS `total_sales`, DATE_FORMAT(`date`, '%Y-%m-%d') AS `formatted_date`, TRUNCATE(SUM(`amount`), 2) AS `total_earned` FROM `payments` WHERE `date` BETWEEN '{$data->start_date}' AND DATE_ADD('{$data->end_date}', INTERVAL 1 DAY) GROUP BY `formatted_date`");
...
$result = $this->database->query("    
    ...
    FROM (
        ...
        FROM `users`
        WHERE `date` BETWEEN '{$data->start_date}' AND DATE_ADD('{$data->end_date}', INTERVAL 1 DAY)
...
$logs_result = $this->database->query("
    ...
    FROM
         `track_links`
    WHERE
         (`date` BETWEEN '{$data->start_date}' AND '{$data->end_date}')
    ORDER BY
        `formatted_date`
");
...
<div class="row mb-5">
    <div class="col-12 col-md mr-md-4 card border-0">
        <div class="card-body">
            ...
            <?php foreach($logs_data['referer'] as $key => $value): ?>
                <div class="row">
                    <div class="col">
                        ...
                        <?php else: ?>
                            <img src="https://www.google.com/s2/favicons?domain=<?= url($key) ?>" class="img-fluid mr-1" />
                            <a href="<?= $key ?>"><?= $key ?></a>
                        <?php endif ?>
    ...
    <div class="col-12 col-md ml-md-4 card border-0">
        <div class="card-body">
...
<div class="mb-5">
    <h2><?= $this->language->admin_statistics->top_packages->header ?></h2>
    ...
    <div class="mt-5 table-responsive table-custom-container">
        <table class="table table-custom">
            ...
            while($row = $result->fetch_object()):
                ?>
                <tr>
                    <td><?= (new \Altum\Models\Package(['settings' => $this->settings]))->getPackageById($row->package_id)->name ?></td>
                    <td><?= $row->total ?></td>
                </tr>
